...
const ALLOWED_GROUPS = ['science', 'admin'];
...
async function populateProgressOrgUnitSelect() {
  ...
  try {
    ...
    const units = (resp.status === 'success' ? (resp.data || []) : []).filter(u => u.group_key === 'science');
  ...
  tbody.innerHTML = projects.map(p => {
    ...
    <tr style="${isSelected ? 'background: var(--hover-bg);' : ''}">
      ...
      <td style="text-align:center;">
        ...
        <button type="button" class="btn btn-outline" title="Ghim vào Cá Nhân" style="padding: 4px 8px; font-size:12px; margin-left:4px;" onclick="pinToPersonalHub('${safeIdArg}', '${escapeHtml(escapeJs(p.name))}')">
...
function renderEventsForSelectedDate() {
  ...
  dailyEvents.forEach(event => {
    ...
    div.innerHTML =
      ...
      '<div class="event-title">' + escapeHtml(event.title) + (recurrenceLabel ? ' <i class="fa-solid fa-rotate" style="font-size:0.75em; color: var(--text-muted);" title="' + recurrenceLabel + '"></i>' : '') + (isGoogleSynced ? ' <i class="fa-brands fa-google" style="font-size:0.75em; color: var(--text-muted);" title="Đồng bộ từ Google Calendar"></i>' : '') + '</div>' +
...
async function loadRoleMgmtTable() {
  ...
  try {
    const [users, roles] = await Promise.all([
      API.auth.getAllUsers('science'),
      API.roles.listAll()
    ]);
    renderRoleMgmtTable(users, roles.filter(r => r.group_key === 'science'));
  } catch (err) {
...
async function updateMemberRoleAction(userId, newRole) {
  try {
    const response = await callGAS('updateMemberRole', { userId, groupKey: 'science', role: newRole });
    if (response.status !== 'success') throw new Error(response.message);
    showToast(response.data || response.message, 'success');
  } catch (err) {
    showToast('Lỗi: ' + err.message, 'error');
...
async function pinToPersonalHub(projectId, projectName) {
  ...
  const res = await window.callGAS('savePersonalItem', {
    type: 'pin',
    title: projectName,
  ...
  if (res.status === 'success') {
    ...
    if (SECTION_LOADED.personal) await refreshPersonalItems();
  }
}
